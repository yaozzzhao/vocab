...
import { ArrowRight, CheckCircle2, XCircle, RotateCcw, BookMarked, Home, Volume2 } from 'lucide-react';
...
const WordInputDisplay: React.FC<{ 
  ...
  for (let i = 0; i < displayWord.length; i++) {
    ...
    if (char === '(' || char === ')') {
      elements.push(<span key={`static-${i}`} className="text-stone-500">{char}</span>);
    } else if (char === ' ') {
      ...
      if (feedbackState === 'correct') {
        ...
        charColor = normalizeWord(displayChar) === normalizeWord(char) ? 'text-green-600' : 'text-red-500';
      }

      elements.push(
        <div key={`input-${i}`} className="relative w-8 h-12 flex items-center justify-center">
          <span className={`z-10 transition-colors duration-150 font-bold ${charColor}`}>
            {displayChar}
          </span>
          {isCursorPosition && feedbackState === 'idle' && <span className="absolute w-0.5 h-8 bg-slate-700 animate-blink z-20"></span>}
  ...
  return (
    <div className={`flex flex-wrap justify-center items-center gap-x-1 text-3xl md:text-4xl font-serif ${isShaking ? 'animate-shake' : ''}`}>
      {elements}
    </div>
  );
};
...
export const TestSession: React.FC<TestSessionProps> = ({ config, onFinish, onAddMistakes, onNavigateHome }) => {
  const [questions, setQuestions] = useState<Word[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [inputValue, setInputValue] = useState('');
  const [answers, setAnswers] = useState<Record<string, { input: string; isCorrect: boolean }>>({});
  ...
  const testableWord = currentWord ? currentWord.word.replace(/[()]/g, '') : '';

  const handleSubmit = (e?: React.FormEvent) => {
    if (e) e.preventDefault();
    if (!currentWord || !inputValue.trim() || feedbackState !== 'idle' || showCorrectAnswer) return;
    ...
    if (isCorrect) {
      ...
      setAnswers(prev => ({ ...prev, [currentWord.id]: { input: inputValue.trim(), isCorrect: true } }));
      setTimeout(() => {
        moveToNextWord();
      }, 1200);
    } else {
      ...
      if (attemptsLeft > 1) {
        ...
        setAnswers(prev => ({ ...prev, [currentWord.id]: { input: inputValue.trim(), isCorrect: false } }));
        if (config.mode === 'unit') {
  ...
  if (!currentWord && !isFinished) return <div className="text-center text-stone-500">Loading test...</div>;
    ...
    return (
      <div className="max-w-3xl mx-auto bg-white rounded-lg border border-stone-200 p-8 sm:p-12">
        <div className="text-center mb-10">
          <h2 className="text-4xl font-serif font-bold text-stone-900 mb-2">Session Complete</h2>
          <p className="text-stone-600 text-lg">
            Your accuracy was <span className="font-bold text-slate-800">{accuracy}%</span> ({correctCount} / {questions.length})
          </p>
        </div>
          ...
          <div className="mb-10">
            ...
            <div className="space-y-4">
              {incorrectWords.map(word => (
                <div key={word.id} className="flex items-start p-4 bg-stone-50 rounded-lg border border-stone-200">
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      ...
                      <p className="text-stone-700 font-medium text-right">{word.meaning}</p>
                    </div>
                    <div className="mt-2 text-sm">
                      <span className="text-stone-500">Your answer: </span>
                      <span className="text-red-600 line-through">{answers[word.id]?.input || '(skipped)'}</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            {config.mode === 'unit' && (
              <p className="text-sm text-stone-500 mt-4 flex items-center">
                ...
                Incorrect words have been automatically added to your Mistake Book.
              </p>
            )}
          </div>
        )}
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="text-center mb-8">
        <p className="text-4xl md:text-5xl font-serif font-bold text-stone-900 mb-4">{currentWord.meaning}</p>
        <div className="flex items-center justify-center gap-3 h-8">
          {currentWord.phonetic && (
            <>
              <p className="text-xl text-stone-500 font-mono">{currentWord.phonetic}</p>
              <button 
      ...
      <div className="text-center text-sm mt-4 h-12">
        {showCorrectAnswer ? (
          <div>
            ...
            <p className="text-2xl font-serif font-bold text-green-600">{currentWord.word}</p>
          </div>
        ) : (
          <p className="text-stone-500 h-5">
            {feedbackState === 'idle' && attemptsLeft < 3 && <span className="text-red-500 font-bold">Incorrect. {attemptsLeft} {attemptsLeft > 1 ? 'attempts' : 'attempt'} left.</span>}
            {feedbackState === 'idle' && attemptsLeft === 3 && <span>Type the word above. Press Enter to submit.</span>}
          </p>
        )}
      </div>
    </div>
